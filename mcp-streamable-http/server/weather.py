"""Weather tools for MCP Streamable HTTP server using NWS (national weather service) API."""
...
# Inialitze FastMCP server for Whether tools.
# if json_response is set to True, the server will use JSON responses instead of SSE streams.
# If stateless_http is set to True, the server uses trye stateless mode (new transprt per request)
mcp = FastMCP(name="weather", json_response=False, stateless_http=False)

NWS_API_BASE = "https://api.weather.gov"
USER_AGENT = "weather-app/1.0"
API_KEY = "weather-app/1.0"

async def make_nws_request(url: str) -> dict[str, Any] | None:
    """Make a request to NWS API with proper error handling."""
    headers = {"User-Agent": USER_AGENT, "API-Key": API_KEY, "ACCEPT": "application/geo+json"}
